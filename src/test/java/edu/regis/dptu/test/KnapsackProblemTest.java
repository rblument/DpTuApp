...
/**
 * Classic 3-item example: A(w=2,v=6), B(w=2,v=10), C(w=3,v=12), W=5. Optimal = 22 (items B + C).
 */
public class KnapsackProblemTest {
    ...
    @Test
    public void testConstructorRejectsNullArrays() {
        assertThrows(
    ...
    @Test
    public void testConstructorRejectsMismatchedArrays() {
        assertThrows(
                IllegalArgumentException.class,
    ...
    @Test
    public void testOptimalValueAfterFullExecution() {
    ...
    @Test
    public void testHasFinishedAfterFullExecution() {
        while (!problem.hasFinished()) problem.step();
        assertTrue(problem.hasFinished());
    }

    @Test
    public void testResetClearsDpTable() {
        problem.step();
        problem.step();
        problem.reset();
        for (int[] row : problem.getDpTable()) for (int cell : row) assertEquals(0, cell);
    }

    @Test
    public void testResetRestoresNotFinished() {
        while (!problem.hasFinished()) problem.step();
        problem.reset();
        assertFalse(problem.hasFinished());
    }
    ...
    @Test
    public void testBacktrackNotReadyAtStart() {
        assertFalse(problem.backtrackReady());
    }

    @Test
    public void testBacktrackReadyAfterForwardExecution() {
        while (!problem.hasFinished()) problem.step();
