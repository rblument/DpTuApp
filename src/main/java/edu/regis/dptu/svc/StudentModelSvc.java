...
/**
 * Specifies the API for {@link StudentModel} life-cycle maintenance (CRUD persistence).
 *
 * @author rickb
 */
public interface StudentModelSvc {
     ...
     * @param field the field(s) in the student model that should be updated in the database.
     * @throws ObjNotFoundException user doesn't exists in the database
     * @throws NonRecoverableException also see getCause().getErrorCode().
     */
    void update(StudentModel model) throws ObjNotFoundException, NonRecoverableException;
    ...
    /**
     * Check if the StudentUser with the given user id exists in the database. (less expensive then
     * using findById).
     *
