 ...
 *
 * <p>Steps in tasks are based on tutoring behaviors in (VanLehn, 2006).
 *
 * <p>As a Step appears within actions/expectations created by an agent, all fields are final so
...
public class Step extends TitledModel {
    ...
    /**
     * The type of this step, which determines the object specified in this step's data along with a
     * potential cont
     */
    ...
    /**
     * The amount of seconds the student can take on this step before the GUI prompts the student
     * concerning their inaction.
     */
    ...
    /**
     * A JSon encoded object specified by out step subtype, which provides the contextual
     * environment (information) related to this step.
     */
    private String data;

    /** The integer value of the correct solution for this step when that solution is an integer. */
    ...
    public Hint findHintById(int id) {
        for (int i = 0; i < hints.size(); i++) if (hints.get(i).getId() == id) return hints.get(i);
