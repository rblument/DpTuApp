...
/**
 * The approaches used to select the next task presented to the student, which follows Van Lehn's
 * outer loop task selections.
 *
 * @author rickb
 */
public enum TaskSelectionKind {
    ...
    /**
     * Units comprise a sequence of difficulty levels and tasks are selected from the current unit
    ...
    /**
     * A default value used in a Knowledge Component indicating that some approach other than
     * macroadaptation is used to select task addressing this component (i.e., a fixed sequence
     * within a unit).
     */
    OTHER("Other"),
    ...
    public static TaskSelectionKind findValue(String aTitle) {
        for (TaskSelectionKind kind : values()) {
            if (kind.getTitle().equalsIgnoreCase(aTitle)) return kind;
        }
