...
public class TutoringSessionView extends GPanel {
     ...
     * <p>This method:
     * <ul>
     *   <li>Replaces the currently displayed {@link ModeView} based on the session's {@link Mode}</li>
     *   <li>Propagates the active {@link edu.regis.dptu.model.Problem} to the selected mode view</li>
     *   <li>Temporarily wires a callback for persisting task completion events to the tutor/server
     *       when running in {@code SEE_ONE} mode</li>
     * </ul>
     *
     * <p><b>Temporary design note:</b> The task completion persistence hook is currently wired through
     * the view layer for expedience. This logic should be migrated to a controller/service layer in a
     * future refactor to improve separation of concerns.
     *
            ...
            seeOne.setOnCompletedTaskSend(() -> {
                try {
                    ...
                    if (model.getTasks().isEmpty()) {
                        log.warn("No current task available/no problem in session; cannot send CompletedTask");
                        return;
                    }

                    int taskId = model.getProblem().getTaskId();
                    if (taskId < 0) {
                        log.warn("Problem.taskId not set (taskId={}); cannot send CompletedTask", taskId);
                        return;
                    }
                    ...
                    log.info("COMPLETED_TASK reply status={} data={}", reply.getStatus(), reply.getData());


                    if (!":OK".equals(reply.getStatus())) {
                        log.warn("CompletedTask failed: status={} data={}", reply.getStatus(), reply.getData());
                    } else {
                        log.info("CompletedTask persisted userId={} taskId={}", userId, taskId);
                    }
