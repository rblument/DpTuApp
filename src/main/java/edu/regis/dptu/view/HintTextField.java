 ...
 *
 * <p>This field also intercepts focus events and moves the caret to offset 0, if the user hasn't
 * entered any data.
 *
 * <p>TODO: Check if newer versions of Java provide this type of field.
 *
 * @author rickb
 */
public class HintTextField extends JTextField {
    ...
    /**
     * True, if the field contains only the default hint text (i.e. no user entered text) and false,
     * otherwise.
     */
    ...
    /**
     * A filter that manages the hint text by removing it when the user has entered one or more
     * legal characters and restoring it when the user has removed all legal characters.
     */
    public class NameFilter extends DocumentFilter {
        ...
        public void insertString(
                DocumentFilter.FilterBypass fb, int offs, String str, AttributeSet s)
                throws BadLocationException {}

        /**
         * If the user enters a non-space initial character, replace the default hint text with the
         * user's character switching to black.
         */
        public void replace(
                DocumentFilter.FilterBypass fb, int offs, int len, String str, AttributeSet s)
                throws BadLocationException {
            if (isFirstEdit) {
                ...
                } else if ((isLetterOrHyphen(str)) || (isEmailAddr && isEmailChar(str))) {
                    isFirstEdit = false;
                    setForeground(Color.BLACK);
                ...
                if ((offs == 0) && (txt.length() == len)) { // replace all txt
                    ...
                    if (isLetterOrHyphen(str) || (isEmailAddr && isEmailChar(str)))
                        super.replace(fb, offs, len, str, s);
                }
            }
        }
